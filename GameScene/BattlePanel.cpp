...
void BattlePanel::InitPlayer()
{
    ...
    player->runAction(GameSettleUp::GetInstance().playerActor.idleAnim);
...
void BattlePanel::update(float delta)
{
    ...
    playerInfo = iter->curPlayer;
    monsterInfo = iter->curMonsters;
    if (iter->isPlayerRound) {
        ...
        auto attackAction = GameSettleUp::GetInstance().playerActor.attackAnim;
        auto updateInfoAction = CallFunc::create([this]() {
            GameSettleUp::GetInstance().playerActor = this->playerInfo;
            GameSettleUp::GetInstance().monsterActors = this->monsterInfo;
        });
        ...
        auto hurtAction = GameSettleUp::GetInstance().playerActor.hurtAnim;
        auto startIdleAction = CallFunc::create([this]() {
            this->player->runAction(GameSettleUp::GetInstance().playerActor.idleAnim);
        });
        auto playerEndAction = CallFunc::create([this]() {
            this->playerOtherAniRunning = false;
        });
